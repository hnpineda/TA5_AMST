...
void loop() {
  ...
  if (axeX <= -0.4 or axeX >= 0.4) {
    ...
    float temp = getTemp();// almaceno la temperatura en un variable float
    // Send Temperature to Sigfox********************************************sending to Sigfox
    Send_Pload((const char*)&temp, sizeof(temp));//envio las direcciones de las variables a la 
                                                 // funcion "Send_Pload" previamente creada
    // Wait 20s********************************************send temperature each 20 seconds?????
    delay(20000);
  }
  delay(1000);
}
